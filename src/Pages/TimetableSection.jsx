...
export default function TimetableSection({ userId }) {
  const storageKey = `sf-timetable-${userId}`;
  ...
  const addEvent = () => {
    ...
    setLectures([...lectures, { id: Date.now(), ...formData }]);
    setFormData({
      subject: "",
      day: dayNames[today.getDay()],
      startTime: "",
  ...
  return (
    <section className="db-panel db-panel--left">
      ...
      {todaysLectures.length === 0 ? (
        <div className="db-empty" onClick={() => setShowModal(true)} style={{ cursor: "pointer" }}>
          <div className="db-empty__icon">+</div>
          <p className="db-empty__text">Add your timetable</p>
        </div>
      ) : (
        <>
